...
# Small helpers
# -----------------------------
def _safe_mode(series, fallback="__MISSING__"):
    m = series.mode(dropna=True)
    return m.iloc[0] if not m.empty else fallback

def _make_buckets(series, n_bins=5):
    if series.dropna().nunique() <= 1:
        return series.astype("object").astype(str)
    try:
        labels = pd.qcut(series, q=n_bins, duplicates="drop")
        return labels.astype(str)
    except Exception:
        labels = pd.cut(series, bins=n_bins, duplicates="drop")
        return labels.astype(str)

# -----------------------------
# GroupModeImputer (stable + cloneable)
# -----------------------------
class GroupModeImputer(BaseEstimator, TransformerMixin):
    """
    Hierarchical categorical imputer with optional extra-group granularity.
    Constructor stores parameters WITHOUT mutating them (sklearn-cloneable).
    Real normalization is done in fit() and stored under private attrs.
    """

    def __init__(self, group_cols=("Brand", "model"), extra_group_cols=None,
                 min_group_size=5, n_bins=5, fallback="__MISSING__"):
        # store exactly what user passed; do not convert types here
        self.group_cols = group_cols
        self.extra_group_cols = extra_group_cols
    ...
    def fit(self, X, y=None):
        ...
        # normalize constructor parameters for internal use
        self._group_cols_list_ = list(self.group_cols) if self.group_cols is not None else []
        self._extra_group_cols_list_ = list(self.extra_group_cols) if self.extra_group_cols is not None else []

        # prepare bucketed extras (store as Series objects)
        self._bucketed_cols_ = {}
        for col in self._extra_group_cols_list_:
            if col in X.columns:
                if pd.api.types.is_numeric_dtype(X[col]):
                    self._bucketed_cols_[col] = _make_buckets(X[col], n_bins=self.n_bins)
                else:
                    self._bucketed_cols_[col] = X[col].astype(str)

        # pair-level group modes (brand,model)
        if all(c in X.columns for c in self._group_cols_list_):
            gp = X.groupby(self._group_cols_list_)
            modes = gp.agg(lambda s: _safe_mode(s, fallback=self.fallback))
            counts = gp.size().rename("group_count")
            self.modes_ = modes.join(counts).reset_index().set_index(self._group_cols_list_)
            if "group_count" in self.modes_.columns:
                self.modes_ = self.modes_[self.modes_["group_count"] >= self.min_group_size]
                self.modes_.drop(columns=["group_count"], inplace=True, errors=True)
        else:
            self.modes_ = pd.DataFrame()

        # first-level modes (brand)
        if self._group_cols_list_ and self._group_cols_list_[0] in X.columns:
            gp1 = X.groupby(self._group_cols_list_[0])
            modes1 = gp1.agg(lambda s: _safe_mode(s, fallback=self.fallback))
            counts1 = gp1.size().rename("group_count")
            self.first_level_modes_ = modes1.join(counts1).reset_index().set_index(self._group_cols_list_[0])
            self.first_level_modes_ = self.first_level_modes_[self.first_level_modes_["group_count"] >= self.min_group_size]
            self.first_level_modes_.drop(columns=["group_count"], inplace=True, errors=True)
        else:
            self.first_level_modes_ = pd.DataFrame()

        # extra granular modes
        self.modes_extra_ = {}
        self.first_level_extra_ = {}
        for col in self._extra_group_cols_list_:
            if col not in X.columns:
                continue
            bucket_series = self._bucketed_cols_.get(col, X[col].astype(str))
            tmp = X.copy()
            tmp["_extra_bucket_"] = bucket_series.values

            if all(c in tmp.columns for c in self._group_cols_list_):
                gp_e = tmp.groupby(self._group_cols_list_ + ["_extra_bucket_"])
                modes_e = gp_e.agg(lambda s: _safe_mode(s, fallback=self.fallback))
                counts_e = gp_e.size().rename("group_count")
                df_e = modes_e.join(counts_e).reset_index()
                df_e = df_e[df_e["group_count"] >= self.min_group_size].drop(columns=["group_count"])
                if not df_e.empty:
                    df_e.set_index(self._group_cols_list_ + ["_extra_bucket_"], inplace=True)
                    self.modes_extra_[col] = df_e

            if self._group_cols_list_ and self._group_cols_list_[0] in tmp.columns:
                gp_e1 = tmp.groupby([self._group_cols_list_[0], "_extra_bucket_"])
                modes_e1 = gp_e1.agg(lambda s: _safe_mode(s, fallback=self.fallback))
                counts_e1 = gp_e1.size().rename("group_count")
                df_e1 = modes_e1.join(counts_e1).reset_index()
                df_e1 = df_e1[df_e1["group_count"] >= self.min_group_size].drop(columns=["group_count"])
                if not df_e1.empty:
                    df_e1.set_index([self._group_cols_list_[0], "_extra_bucket_"], inplace=True)
                    self.first_level_extra_[col] = df_e1

        # global fallback
        self.global_mode_ = X.apply(lambda s: _safe_mode(s, fallback=self.fallback))
        return self

    def transform(self, X):
        X = pd.DataFrame(X).copy()

        # use internal normalized lists if available, else fallback to constructor values
        group_cols = getattr(self, "_group_cols_list_", list(self.group_cols) if self.group_cols is not None else [])
        extra_cols = getattr(self, "_extra_group_cols_list_", list(self.extra_group_cols) if self.extra_group_cols is not None else [])

        # build bucketed maps for transform (recompute bucketing pragmatically)
        bucket_maps = {}
        for col in extra_cols:
            if col not in X.columns:
                continue
            if col in getattr(self, "_bucketed_cols_", {}) and pd.api.types.is_numeric_dtype(X[col]):
                bucket_maps[col] = _make_buckets(X[col], n_bins=self.n_bins)
            else:
                bucket_maps[col] = X[col].astype(str)

        cols_to_impute = [c for c in X.columns if c not in group_cols and X[c].isna().any()]
        if not cols_to_impute:
            return X.values

        # helper: prepare key_df if (brand,model) present
        key_df = X[group_cols].copy() if group_cols and all(c in X.columns for c in group_cols) else None

        # 1) extras (brand,model,extra) and (brand,extra)
        for col in extra_cols:
            if col not in X.columns:
                continue
            bucket_series = bucket_maps[col]
            tmp_key = X[group_cols].copy() if key_df is not None else pd.DataFrame()
            tmp_key["_extra_bucket_"] = bucket_series.values

            if col in getattr(self, "modes_extra_", {}) and not self.modes_extra_[col].empty:
                mode_df = self.modes_extra_[col].reset_index()
                joined = tmp_key.merge(mode_df, on=group_cols + ["_extra_bucket_"], how="left")
                for c in cols_to_impute:
                    if c not in self.modes_extra_[col].columns:
                        continue
                    mask = X[c].isna() & joined[c].notna()
                    X.loc[mask, c] = joined.loc[mask, c]

            if col in getattr(self, "first_level_extra_", {}) and not self.first_level_extra_[col].empty:
                mode1 = self.first_level_extra_[col].reset_index()
                join_left = X[[group_cols[0]]].assign(_extra_bucket_=bucket_series.values)
                joined1 = join_left.merge(mode1, on=[group_cols[0], "_extra_bucket_"], how="left")
                for c in cols_to_impute:
                    if c not in mode1.columns:
                        continue
                    mask = X[c].isna() & joined1[c].notna()
                    X.loc[mask, c] = joined1.loc[mask, c]

        # 2) pair-level (brand,model)
        if key_df is not None and not getattr(self, "modes_", pd.DataFrame()).empty:
            mode_df = self.modes_.reset_index()
            joined = key_df.merge(mode_df, on=group_cols, how="left")
            for c in cols_to_impute:
                if c not in self.modes_.columns:
                    continue
                mask = X[c].isna() & joined[c].notna()
                X.loc[mask, c] = joined.loc[mask, c]

        # 3) first-level (brand)
        if group_cols and group_cols[0] in X.columns and not getattr(self, "first_level_modes_", pd.DataFrame()).empty:
            g = group_cols[0]
            mode1 = self.first_level_modes_.reset_index()
            joined1 = X[[g]].merge(mode1[[g] + [c for c in cols_to_impute if c in mode1.columns]], on=g, how="left")
            for c in cols_to_impute:
                if c not in mode1.columns:
                    continue
                mask = X[c].isna() & joined1[c].notna()
        ...
        # 4) global fallback
        for c in cols_to_impute:
            X[c] = X[c].fillna(self.global_mode_.get(c, self.fallback))
...
# -----------------------------
# GroupMedianImputer (stable + cloneable)
# -----------------------------
class GroupMedianImputer(BaseEstimator, TransformerMixin):
    """
    Hierarchical median imputer with optional extras.
    Constructor stores parameters without mutation. Normalization done in fit().
    """

    def __init__(self, group_cols=("Brand", "model"), extra_group_cols=None, min_group_size=5, n_bins=5):
        self.group_cols = group_cols
        self.extra_group_cols = extra_group_cols
        self.min_group_size = int(min_group_size)
        self.n_bins = int(n_bins)
    ...
    def fit(self, X, y=None):
        ...
        # numeric columns to consider (exclude group cols)
        numeric_cols = X.select_dtypes(include=[np.number]).columns.tolist()
        self._numeric_cols_ = [c for c in numeric_cols if c not in self._group_cols_list_]
        ...
        # pair-level medians
        if all(c in X.columns for c in self._group_cols_list_):
            self.medians_ = X.groupby(self._group_cols_list_).median(numeric_only=True)
            counts = X.groupby(self._group_cols_list_).size().rename("group_count")
            self.medians_ = self.medians_.join(counts).reset_index()
            self.medians_ = self.medians_[self.medians_["group_count"] >= self.min_group_size].drop(columns=["group_count"])
            self.medians_.set_index(self._group_cols_list_, inplace=True)
        else:
            self.medians_ = pd.DataFrame()

        # first-level medians
        if self._group_cols_list_ and self._group_cols_list_[0] in X.columns:
            self.first_level_medians_ = X.groupby(self._group_cols_list_[0]).median(numeric_only=True)
            counts1 = X.groupby(self._group_cols_list_[0]).size().rename("group_count")
            self.first_level_medians_ = self.first_level_medians_.join(counts1).reset_index()
            self.first_level_medians_ = self.first_level_medians_[self.first_level_medians_["group_count"] >= self.min_group_size].drop(columns=["group_count"])
            self.first_level_medians_.set_index(self._group_cols_list_[0], inplace=True)
        else:
        ...
        for col in self._extra_group_cols_list_:
            ...
            if all(c in tmp.columns for c in self._group_cols_list_):
                med_e = tmp.groupby(self._group_cols_list_ + ["_extra_bucket_"]).median(numeric_only=True)
                counts_e = tmp.groupby(self._group_cols_list_ + ["_extra_bucket_"]).size().rename("group_count")
                df_e = med_e.join(counts_e).reset_index()
                df_e = df_e[df_e["group_count"] >= self.min_group_size].drop(columns=["group_count"])
                if not df_e.empty:
                    df_e.set_index(self._group_cols_list_ + ["_extra_bucket_"], inplace=True)
                    self.medians_extra_[col] = df_e

            if self._group_cols_list_ and self._group_cols_list_[0] in tmp.columns:
                med_e1 = tmp.groupby([self._group_cols_list_[0], "_extra_bucket_"]).median(numeric_only=True)
                counts_e1 = tmp.groupby([self._group_cols_list_[0], "_extra_bucket_"]).size().rename("group_count")
                df_e1 = med_e1.join(counts_e1).reset_index()
                df_e1 = df_e1[df_e1["group_count"] >= self.min_group_size].drop(columns=["group_count"])
                if not df_e1.empty:
                    df_e1.set_index([self._group_cols_list_[0], "_extra_bucket_"], inplace=True)
                    self.first_level_extra_[col] = df_e1
    ...
    def transform(self, X):
        ...
        for col in extra_cols:
            ...
            if col in getattr(self, "medians_extra_", {}) and not self.medians_extra_[col].empty:
                med_df = self.medians_extra_[col].reset_index()
                tmp_key = X[group_cols].assign(_extra_bucket_=bucket_series.values) if key_df is not None else pd.DataFrame()
                joined = tmp_key.merge(med_df, on=group_cols + ["_extra_bucket_"], how="left")
                for c in cols_to_impute:
                    if c not in self.medians_extra_[col].columns:
                        continue
                    mask = X[c].isna() & joined[c].notna()
                    X.loc[mask, c] = joined.loc[mask, c]

            if col in getattr(self, "first_level_extra_", {}) and not self.first_level_extra_[col].empty:
                med1 = self.first_level_extra_[col].reset_index()
                joined1 = X[[group_cols[0]]].assign(_extra_bucket_=bucket_series.values).merge(
                    med1[[group_cols[0], "_extra_bucket_"] + [c for c in cols_to_impute if c in med1.columns]],
                    on=[group_cols[0], "_extra_bucket_"],
                    how="left"
                )
                for c in cols_to_impute:
                    if c not in med1.columns:
                        continue
                    mask = X[c].isna() & joined1[c].notna()
                    X.loc[mask, c] = joined1.loc[mask, c]

        # pair-level
        if key_df is not None and not getattr(self, "medians_", pd.DataFrame()).empty:
            med_df = self.medians_.reset_index()
            joined = key_df.merge(med_df, on=group_cols, how="left")
            for c in cols_to_impute:
                if c not in self.medians_.columns:
                    continue
                mask = X[c].isna() & joined[c].notna()
                X.loc[mask, c] = joined.loc[mask, c]

        # first-level
        if group_cols and group_cols[0] in X.columns and not getattr(self, "first_level_medians_", pd.DataFrame()).empty:
            g = group_cols[0]
            med1 = self.first_level_medians_.reset_index()
            joined1 = X[[g]].merge(med1[[g] + [c for c in cols_to_impute if c in med1.columns]], on=g, how="left")
            for c in cols_to_impute:
                if c not in med1.columns:
                    continue
                mask = X[c].isna() & joined1[c].notna()
        ...
        # global fallback
        for c in cols_to_impute:
            if c in self.global_median_:
                X[c] = X[c].fillna(self.global_median_[c])
